...

{
  imports = [
    ...
    ./../../modules/extra/matrix/element.nix #Matrix Client for Gnome
...
#/* HARDWARE STUFF
 # Enable OpenGL
  ...
  hardware.nvidia = {
    ...
    package = config.boot.kernelPackages.nvidiaPackages.legacy_580; # Older versions
  ...
  users.users.personal = {
    ...
    extraGroups = [ "networkmanager" "wheel" "video" "docker" "monica"];
    packages = with pkgs; [
    librewolf
    ];
    shell = pkgs.fish;
